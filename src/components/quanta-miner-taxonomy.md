...
| Dimension | Options |
|-----------|---------|
| **Registration** | Solo (Direct UID) · Pooled (via Operator) |
| **Generation Method** | Discretionary · Systematic · Hybrid |
| **Operator Scale** | Individual · Team · Platform |
| **Interface Type** | CLI · Web · Mobile · API · Social · Embedded |
...
- **Compute**: Minimal—spreadsheets, research databases
...
- **Compute**: Minimal—event tracking tools
...
- **Interface**: Python scripts → JSON export, API submission
...
- **Interface**: MLOps pipeline → portfolio extraction → API
...
- **Interface**: Data pipeline → signal extraction → portfolio weights
- **Compute**: High
...
- **Interface**: Agent inference → action space → portfolio weights
...
- **Interface**: Agent orchestration → structured output → portfolio JSON
...
- **Aggregation Method**: Market prices → implied probability → weights
...
- **User Interface**:
  - Research paper → strategy conversion
...
- **Aggregation Method**: Analyst conviction × quant factor score
...
Low Barrier                                              High Barrier
    │                                                         │
    ▼                                                         ▼
[QUANTA Web GUI] → [QUANTA CLI] → [QUANTA API] → [Numerai] → [Prop Trading]
    │                   │              │             │            │
...
This accessibility—combined with the Signal Pool architecture for unlimited scale—positions QUANTA to aggregate intelligence from the broadest possible participant base, maximizing the network's collective alpha generation potential.
