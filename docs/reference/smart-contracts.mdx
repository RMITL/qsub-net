...
| Symbol | ALPHA |
| Decimals | 18 |
| Max Supply | 21,000,000 (Bittensor dTAO cap) |
| Emission Model | Protocol-controlled (not pre-minted) |

<Note>
Unlike traditional ERC-20 tokens, QUANTA's α-token follows Bittensor's dTAO emission model. Tokens are emitted by the protocol based on network activity, not pre-minted or allocated.
</Note>
