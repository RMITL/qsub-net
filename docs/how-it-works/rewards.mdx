...
<Note>
Tier thresholds and distribution percentages are **governance-tunable**. The values shown represent the launch configuration.
</Note>
...
All Antes Collected
        ...
        ▼
┌───────────────────┐
│  Optional Rake    │ ──── 0-8% to treasury (governance-tunable)*
│     (0-8%)        │
└───────────────────┘
        │
        ▼
...
└───────────────────┘
        │
        ├─── Bottom 20%: Forfeit ante
        │         ├── 50% burned (deflationary)
        │         └── 50% to winner pool
        │
        ├─── Break-even 25%: Ante returned
        │
        ├─── Profitable 45%: Ante + bonus
        │
        └─── Top 10%: Ante + premium bonus
```

<Note>
*Setting the network rake to 0% is under active consideration. With TAO emissions (18% owner share) and external revenue providing sustainable network funding, 100% of the ante pool may be redistributed to winning signal generators.
</Note>
...
- Optional network rake (0-8%): 0-120 α to treasury*
- Remaining pot: 1,380-1,500 α

*Setting rake to 0% is under consideration.

**After Performance Evaluation:**
```
Bottom 20% (20 signals, ~300 α ante):
  - Burned: 150 α
  - To winners: 150 α
...
Profitable 45% (45 signals):
  - Pool share: (150 × 0.40) / 45 ≈ 1.3 α bonus each

Top 10% (10 signals):
  - Pool share: (150 × 0.50) / 10 = 7.5 α bonus each
```
