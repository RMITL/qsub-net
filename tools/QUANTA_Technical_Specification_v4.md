...
- **Multi-Horizon Evaluation**: Rolling windows of 7, 30, and 90 days capture both short-term alpha and long-term consistency, weighted 20%/30%/50% respectively to prioritize sustainable performance.
...
1. **Validator Staking**: Required collateral (minimum 10,000α) to participate in validation
...
Performance evaluation
        ↓
Bottom 20-30% forfeit ante → 50% burned, 50% to winner pool
...
- 6.1 Multi-Horizon Framework
  - 6.1.1 Short-Term Window (7 days, 20% weight)
  - 6.1.2 Medium-Term Window (30 days, 30% weight)
  - 6.1.3 Long-Term Window (90 days, 50% weight)
...
### 11. Risk Analysis
- 11.1 Market Risks
  - 11.1.1 Equities Market Volatility
  - 11.1.2 Correlation Breakdown
  - 11.1.3 Regime Changes
- 11.2 Protocol Risks
  - 11.2.1 Consensus Failures
  - 11.2.2 Oracle Manipulation
  - 11.2.3 Smart Contract Vulnerabilities
- 11.3 Economic Risks
  - 11.3.1 Token Volatility
  - 11.3.2 Liquidity Constraints
  - 11.3.3 Incentive Misalignment
- 11.4 Regulatory Risks
  - 11.4.1 Securities Law Considerations
  - 11.4.2 Decentralized Finance Regulation
  - 11.4.3 Compliance Strategy
- 11.5 Mitigation Strategies
  - 11.5.1 Technical Safeguards
  - 11.5.2 Economic Buffers
  - 11.5.3 Legal Structuring
  - 11.5.4 Insurance Mechanisms

### 12. Roadmap and Milestones
- 12.1 Development Phases
  - 12.1.1 Phase 1: Protocol Design and Testnet (Months 1-3)
  - 12.1.2 Phase 2: Mainnet Launch (Months 4-6)
  - 12.1.3 Phase 3: Ecosystem Growth (Months 7-12)
  - 12.1.4 Phase 4: Multi-Asset Expansion (Months 13-24)
- 12.2 Technical Milestones
  - 12.2.1 Smart Contract Audits
  - 12.2.2 Testnet Performance Benchmarks
  - 12.2.3 Mainnet Deployment
  - 12.2.4 API and Infrastructure Releases
- 12.3 Ecosystem Milestones
  - 12.3.1 Miner Onboarding Targets
  - 12.3.2 Validator Network Growth
  - 12.3.3 Partnership Announcements
  - 12.3.4 Integration Completions
- 12.4 Token Milestones
  - 12.4.1 Initial Liquidity Events
  - 12.4.2 Exchange Listings
  - 12.4.3 Market Cap Targets
  - 12.4.4 Staking Participation Rates

### 13. Team and Advisors
- 13.1 Core Team
- 13.2 Advisory Board
- 13.3 Partners and Collaborators
- 13.4 Open Source Contributors
...
- Appendix A: Mathematical Formulations
  - A.1 Sortino Ratio Calculation
  - A.2 Calmar Ratio Calculation
  - A.3 Maximum Drawdown Computation
  - A.4 Turnover Penalty Function
  - A.5 Composite Score Aggregation
  - A.6 Yuma Consensus Mathematics
  - A.7 Reward Distribution Formulas
- Appendix B: Signal Examples
  - B.1 Market Neutral Portfolio
  - B.2 Momentum Strategy
  - B.3 Value Strategy
  - B.4 Multi-Factor Composite
- Appendix C: Code Snippets
  - C.1 Signal Submission (Python)
  - C.2 Validator Evaluation (Python)
  - C.3 Smart Contract Interfaces (Solidity)
- Appendix D: Economic Simulations
  - D.1 Token Price Scenarios
  - D.2 Emission Schedule Analysis
  - D.3 Staking Equilibrium Models
- Appendix E: Glossary
- Appendix F: References and Citations
- Appendix G: Legal Disclaimers
...
**Current Status:** FINAL DRAFT v3.0
**Next Review:** January 2026
**Public Comments:** Open until December 15, 2025
...
**Errata and Updates:**
Visit https://cquant.co/docs/tech-spec for the latest version and known errata.
...
| `ante_amount` | float | Yes | Stake amount in alpha-tokens | Minimum 10.0 α-tokens |
...
$$
H_{\text{commit}} = \text{keccak256}(H_{\text{portfolio}} \parallel s \parallel a_{\text{miner}} \parallel c_{\text{chain}} \parallel e_{\text{epoch}})
$$

Where:
- $H_{\text{portfolio}}$ = keccak256 hash of concatenated tickers and weights
- $s$ = 256-bit secret salt
- $a_{\text{miner}}$ = Miner's Bittensor hotkey address
- $c_{\text{chain}}$ = Subnet chain identifier
- $e_{\text{epoch}}$ = Current epoch ID
...
|--------------|----------------|----------|
| 6-12 hours | Minimum | High-frequency rebalancing strategies |
| 14-24 hours | Recommended | Standard weekly portfolio submissions |
| 24-48 hours | Maximum Security | High-stakes competitions, quarterly epochs |
...
def verify_commitment(reveal_data, original_commitment):
    ...
    assert 300 <= (reveal_block - commitment_block) <= 24000  # 6-48 hours
...
**48-Hour Expiration Rule**:
- Reveals submitted >48 hours after commitment are **automatically invalidated**
- Ante is **forfeited** and distributed to validator pool
...
| Expired reveal (>48h) | 100% ante forfeiture | High |
...
- Ante: 50 α-tokens (above 10 minimum)
...
- Lower weight prevents overemphasis on noise/luck

**Medium-Term (30-Day) - 45% Weight (Highest)**:
...
- Dampens impact of single exceptional week
...
def calculate_daily_quanta_score(miner_id, evaluation_date):
    ...
    # Composite score
    total_score = 0.30 * score_7d + 0.45 * score_30d + 0.25 * score_90d
...
| 6.15 | Normalization constant | 6.5.4 |
...
With correlation threshold τ = 0.7 and penalty coefficient γ sufficiently large:
...
QUANTA sets a correlation threshold τ = 0.7 for flagging derivative strategies.
...
| ρ = 0.7 | 16.3% | Highly correlated (threshold) |
| ρ = 0.9 | 5.1% | Essentially duplicate |

At ρ = 0.7, the diversification benefit drops below 20%, indicating diminishing marginal value from the second strategy.
...
At τ = 0.7, mutual information exceeds 0.25 bits, indicating substantial signal overlap.
...
Penalty_1 = 2.5 · (0.85 - 0.7)² · 8.5 = 2.5 · 0.0225 · 8.5 = 0.478
Penalty_2 = 2.5 · (0.75 - 0.7)² · 8.5 = 2.5 · 0.0025 · 8.5 = 0.053

Score_final(X) = 8.5 - 0.478 - 0.2 · 0.053 = 8.01
```

A ~6% score reduction from correlation penalties.
...
curl -X POST https://pool.cquant.co/v1/signals/submit \
...
scrape_configs:
  - job_name: 'validator_node'
    static_configs:
      - targets: ['validator1.cquant.co:9090']
...
**Ante**: Minimum stake required to submit signal (10 α-tokens). Acts as spam prevention and skin-in-the-game mechanism.
...
$$\delta = \sqrt{\frac{1}{n} \sum_{i=1}^{n} \min(R_i - \text{MAR}, 0)^2}$$

**Drawdown**: Peak-to-trough decline in portfolio value.
$$\text{DD}(t) = \frac{\text{Peak}(t) - \text{Value}(t)}{\text{Peak}(t)}$$
...
**QUANTA Score (QS)**: Composite performance metric combining Sortino (35%), Calmar (25%), Drawdown (25%), Turnover (15%) across three time horizons.
$$QS = 0.30 \cdot QS_{7d} + 0.45 \cdot QS_{30d} + 0.25 \cdot QS_{90d}$$
...
**Sortino Ratio**: Excess return per unit of downside volatility. Preferred over Sharpe for asymmetric return distributions.
$$\text{Sortino} = \frac{R_p - \text{MAR}}{\delta_d}$$
...
$$\text{Turnover} = \sum_{t=1}^{T} \sum_{i=1}^{N} |w_{i,t} - w_{i,t-1}|$$
...
**Appendices Summary:**
- **Appendix A**: Complete JSON schemas for signals, validator submissions, oracle data, and API responses
- **Appendix B**: Mathematical proofs of incentive compatibility, Sybil resistance, Byzantine fault tolerance, and economic security
- **Appendix C**: Security audit checklist covering smart contracts, consensus, oracles, and penetration testing
- **Appendix D**: Competitive analysis matrix with detailed feature comparisons and strategic positioning
- **Appendix E**: Comprehensive glossary of 50+ technical terms
- **Appendix F**: 43 academic and industry references in IEEE format
- **Appendix G**: Risk disclosures covering technical, market, regulatory, operational, and competitive risks

**Total Length**: ~23,000 words (approximately 20-22 pages when formatted)
...
*For the latest version of this document and errata, visit:*
**https://cquant.co/docs/tech-spec**
